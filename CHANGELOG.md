...
- **Connection heartbeat (the headline feature).** A new **Heartbeat** field (seconds, default `30`, `0` disables) sends a WebSocket ping once the connection is authenticated, and terminates the link once two consecutive probes have gone unanswered (detected on the third interval) with no pong *and* no other inbound traffic. Any traffic counts as evidence of life, so a server that streams data but ignores pings is never treated as dead.
...
- **The documented heartbeat detection window was wrong** in three places: the interval increments and then compares, so termination happens once two probes have gone unanswered — on the third interval, not the second.
