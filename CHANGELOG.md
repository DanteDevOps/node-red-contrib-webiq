...
- **A login that timed out did not count as an attempt** and reconnected on the *fast* transport ladder, producing roughly 2,400 logins a day against a slow project with no misconfiguration involved at all. It now spends the same budget and uses the slow ladder.
